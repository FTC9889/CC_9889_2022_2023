...
@Config
public class ScanForHub extends OpenCvPipeline {
    public static int area = 300;
    ...
    private ArrayList<MatOfPoint> hullContours = new ArrayList<MatOfPoint>();

    public static HSV blueHSV = new HSV(80, 110,
            90, 200, 0, 255);
    public static HSV hsv = new HSV(0, 70,
            80, 255, 0, 100);

    private Point point = new Point(1e10, 1e10);

    public static boolean outPutMask = false;
...
//        Imgproc.rectangle(blurOutput, new Point(0, 0), new Point(320, 25), new Scalar(0, 0, 0), -1);
        ...
        convexHulls(contours, hullContours);

        List<Moments> mu = new ArrayList<>();

        for (int i = 0; i < hullContours.size(); i++) {
            mu.add(Imgproc.moments(hullContours.get(i)));
        }
        ...
        for (int i = 0; i < hullContours.size(); i++) {
            Imgproc.drawContours(cvResizeOutput, hullContours, i, new Scalar(0, 255, 0), -1);
        }
        ...
        cvResize(hsvThresholdOutput, cvResizeDsize, 4, 4, cvResizeInterpolation, hsvThresholdOutput);
        cvResize(cvResizeOutput, cvResizeDsize, 4, 4, cvResizeInterpolation, cvResizeOutput);
